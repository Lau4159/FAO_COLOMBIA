...
# (FIX) usar texp_col_candidates, no tenf_col_candidates
...
server <- function(input, output, session){
  ...
  output$ttl_mapa_e1   <- renderText({ paste("Mapa por", indic_lbl_e1()) })
  output$ttl_origen_e1 <- renderText({ "Origen de la enfermedad — conteo de casos" })
  output$ttl_top_e1    <- renderText({ paste("Top 12 municipios por", tolower(indic_lbl_e1())) })
  ...
  observe({
    ...
    if (nivel_e1()=="deptos") {
      ...
      pal <- make_pal(shp$valor, if (input$f_indic_e1=="incid") "PuBuGn" else "YlGnBu")
      ...
      pal <- make_pal(shp$valor, if (input$f_indic_e1=="incid") "PuRd" else "YlOrRd")
      bb  <- sf::st_bbox(shp)
      leaflet::leafletProxy("map_eta_e1", data=shp) %>%
        leaflet::clearShapes() %>% leaflet::clearControls() %>%
  ...
  output$ttl_incid_bar_e2 <- renderText({
    if (is.null(input$f_depto_e2) || input$f_depto_e2 == "Todos") paste0("Incidencia (x100k) por departamento — Año ", input$f_anio_e2)
    else paste0("Incidencia (x100k) por municipio — ", input$f_depto_e2, " — Año ", input$f_anio_e2)
  })
  ...
  output$ttl_genero_e2 <- renderText({
    if (is.null(input$f_depto_e2) || input$f_depto_e2 == "Todos") paste0("Distribución por género — País — Año ", input$f_anio_e2)
    else paste0("Distribución por género — ", input$f_depto_e2, " — Año ", input$f_anio_e2)
  })
  ...
  output$ttl_exp_enf_map_e2 <- renderText({
    if (is.null(input$f_depto_e2) || input$f_depto_e2 == "Todos") paste0("Enfermos / Expuestos (%) — por departamento — Año ", input$f_anio_e2)
    else paste0("Enfermos / Expuestos (%) — por municipio — ", input$f_depto_e2, " — Año ", input$f_anio_e2)
  })
  ...
  # (FIX) no usar validate/need dentro del reactive; usar req()
  ...
  observe({
    ...
    if (is.null(input$f_depto_e2) || input$f_depto_e2=="Todos") {
      ...
      pal  <- leaflet::colorBin("PuBuGn", domain=dd$ratio, bins=bins, na.color="#f0f0f0")
      ...
      leaflet::leafletProxy("map_exp_enf_e2", data=shp) %>%
        leaflet::clearShapes() %>% leaflet::clearControls() %>%
        leaflet::addPolygons(layerId=~COD_DPTO2, fillColor=~pal(ratio), color="#666", weight=0.7, fillOpacity=0.9,
                             label=~lapply(etq, htmltools::HTML),
                             highlightOptions=leaflet::highlightOptions(color="black", weight=2, bringToFront=TRUE)) %>%
      ...
      # Si cambia a "Todos" mientras se ejecuta, evitar errores
      if (is.null(input$f_depto_e2) || input$f_depto_e2=="Todos") return(NULL)
      ...
      leaflet::leafletProxy("map_exp_enf_e2", data=shp) %>%
        leaflet::clearShapes() %>% leaflet::clearControls() %>%
        leaflet::addPolygons(layerId=~COD_MUN5, fillColor=~pal(ratio), color="#666", weight=0.4, fillOpacity=0.9,
                             label=~lapply(etq, htmltools::HTML),
                             highlightOptions=leaflet::highlightOptions(color="black", weight=2, bringToFront=TRUE)) %>%
        leaflet::addLegend("bottomright", pal=pal, values=dd$ratio,
                           title="Enfermos / Expuestos (%)",
