...
# ---------- Rutas ----------
local_data_dir <- "/Users/jorgearmandoruedagallardo/Library/CloudStorage/OneDrive-SharedLibraries-Universidaddelosandes/FAO-SAT - Documentos/General/06_Scripts/03_SNINNY_APP/SIVIGILA_BPAN/data"
...
# 🔒 Filtro global: quedarnos con filas donde SANTANDER es ORIGEN o DESTINO
bpan <- bpan %>%
  dplyr::filter(
    (toupper(trimws(dplyr::coalesce(DEP_O, ""))) == "SANTANDER") |
      (toupper(trimws(dplyr::coalesce(DEP_D, ""))) == "SANTANDER")
  )
        ...
        tabPanel(
          ...
          div(class="filters",
              div(class="filters-grid",
                  div(class="filter", div(class="filter-label","Año"), uiOutput("anio_bpan_ui")),
                  div(class="filter", div(class="filter-label","Departamento (Ocurrencia)"),
                      selectInput("f_depto_bpan", NULL, choices = "SANTANDER", selected = "SANTANDER")),
                  div(class="filter", div(class="filter-label","Municipio (Ocurrencia)"),
                      selectInput("f_mpio_bpan", NULL, choices = "Todos", selected = "Todos")),
                  div(class="filter", div(class="filter-label","Departamento (Notificación)"),
                      selectInput("f_depto_dest", NULL, choices = "SANTANDER", selected = "SANTANDER")),
          ...
          fluidRow(
            column(
              width = 6,
              div(class="card",
                  div(class="card-title","BPAN — Mapa por Casos (filas)"),
...
server <- function(input, output, session){
  ...
  top10_df <- reactive({
    if (map_nivel() == "deptos") {
      bpan_agg_depto() %>% dplyr::mutate(nombre = ifelse(!is.na(DEP_D) & nzchar(DEP_D), DEP_D, COD_DPTO2)) %>% dplyr::select(nombre, valor) %>%
        dplyr::arrange(dplyr::desc(valor)) %>% dplyr::slice_head(n = 10)
    } else {
      bpan_agg_mpio() %>% dplyr::filter(COD_DPTO2 == sel_cod_dep()) %>% dplyr::mutate(nombre = ifelse(!is.na(MUN_D) & nzchar(MUN_D), MUN_D, COD_MUN5)) %>%
  ...
  output$bar_bpan <- renderPlotly({
    ...
    plot_ly(data = df %>% dplyr::arrange(valor),
            ...
            hovertemplate = paste0("%{y}<br>Casos: %{x:,}<extra></extra>")) %>%
