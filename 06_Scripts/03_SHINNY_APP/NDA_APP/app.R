...
normalize_sex <- function(x){
  ...
  dplyr::case_when(
    x %in% c("M","MASCULINO","HOMBRE","1") ~ "Niño (M)",
    x %in% c("F","FEMENINO","MUJER","2")   ~ "Niña (F)",
    TRUE                                   ~ NA_character_
  )
}
        ...
        tabPanel(
          ...
          div(class="filters",
              div(class="filters-grid",
                  ...
                  div(class="filter", div(class="filter-label","Sexo del niño/a"),
                      selectInput("f_sexo_tab1", NULL, choices = c("Todos","Niño (M)","Niña (F)"), selected = "Todos")),
          ...
          fluidRow(
            column(
              width = 6,
              div(class="card",
                  div(class="card-title","NDA — Mapa por Casos (filas)"),
            ...
            column(
              width = 6,
              div(class="card",
                  div(class="card-title","Top-10 destinos por casos"),
              ...
              div(
                ...
                div(
                  class = "card-title",
                  HTML(
                    'Principales municipios DESTINO (desde el ORIGEN, excluye el mismo depto)
                     <span class="info-badge"
                           data-bs-toggle="tooltip"
                           data-bs-placement="left"
                           title="Top-15 de municipios destino a los que llegan los casos desde el DEPARTAMENTO (ORIGEN) seleccionado.
Siempre excluye los municipios cuyo DEPARTAMENTO DESTINO sea el mismo que el de ORIGEN.
Si eliges un Departamento (DESTINO), se filtra a ese departamento (opcional).">
                     i</span>'
                  )
                ),
        ...
        tabPanel(
          ...
          div(class="filters",
              div(class="filters-grid",
                  ...
                  div(class="filter", div(class="filter-label","Sexo del niño/a"),
                      selectInput("f_sexo_tab2", NULL, choices = c("Todos","Niño (M)","Niña (F)"), selected = "Todos"))
              )
          ),
          fluidRow(
            column(6,
                   ...
                   div(class="card",
                       div(class="card-title","Casos NDA por grupos de edad del niño/a"),
                       plotlyOutput("edad_nna_barras", height = 300)
                   ),
                   div(class="card",
                       div(class="card-title","Distribución por sexo del niño/a"),
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              width = 6,
              div(class="card",
                  div(class="card-title","Serie mensual de casos de Supervivencia y Defunciones"),
                  plotlyOutput("serie_anual_tab3", height = 320)
              ),
              div(class="card",
                  div(class="card-title","Distribución de edad del niño/a (solo fallecidos)"),
            ...
            column(
              ...
              div(class="card",
                 ...
                 <span class="info-badge" data-bs-toggle="tooltip" data-bs-placement="left"
                       title="Se calcula como: Vivos / (Vivos + Muertes). 
Si eliges un departamento destino, el mapa se desagrega a municipios.">i</span>')
                  ),
...
server <- function(input, output, session){
  ...
  observe({
    ...
    if (map_nivel()=="deptos") {
      ...
      pal <- make_pal(shp$valor, "YlOrRd"); bb  <- sf::st_bbox(shp)
      leaflet::leafletProxy("map_nda", data=shp) %>% leaflet::clearShapes() %>% leaflet::clearControls() %>%
  ...
  output$destinos_desde_origen <- renderPlotly({
    ...
    destino_sel <- if (!is.null(input$f_depto_dest) && input$f_depto_dest != "Todos")
      input$f_depto_dest else NA_character_
    
    df2 <- df %>%
      dplyr::mutate(
        destino = factor(destino, levels = rev(destino)),
        color   = dplyr::if_else(!is.na(destino_sel) & NUP(destino) == NUP(destino_sel),
                                 "#2563eb", "#94a3b8")
      )
  ...
  destinos_mpio_df <- reactive({
    ...
    if (!is.null(input$f_mpio_dest) && input$f_mpio_dest != "Todos") {
      out <- out %>%
        dplyr::mutate(color = dplyr::if_else(
          NUP(gsub(" \\(.+\\)$","", destino_lbl)) == NUP(input$f_mpio_dest),
          "#2563eb", "#94a3b8"))
    } else {
      out <- out %>% dplyr::mutate(color = "#94a3b8")
    }
    out
  })
  ...
  # ---- (NUEVO) BARRAS: grupos de edad (Tab 2) ----
  ...
  # ---- (NUEVO) BARRAS: distribución por sexo (Tab 2) ----
  output$sexo_nna_barras <- renderPlotly({
    df <- base_b2() %>%
      dplyr::mutate(sexo_cat = dplyr::case_when(
        sexo_ni %in% c("Niño (M)","Niña (F)") ~ sexo_ni,
        TRUE ~ "Sin dato"
      )) %>%
      dplyr::group_by(sexo_cat) %>%
      dplyr::summarise(Casos = dplyr::n(), .groups = "drop") %>%
      dplyr::arrange(match(sexo_cat, c("Niño (M)","Niña (F)","Sin dato")))
    ...
    plot_ly(df, x = ~Casos, y = ~sexo_cat, type = "bar", orientation = "h",
      ...
      layout(
        xaxis = list(title = "Casos"),
        yaxis = list(title = "Sexo del niño/a"),
  ...
  output$sexo_b3_ui <- renderUI({
    selectInput("f_sexo_b3", "", choices = c("Todos","Niño (M)","Niña (F)"), selected = "Todos")
  })
