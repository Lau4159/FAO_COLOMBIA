# app_idm.R — Terridata IDM (3 cuadrantes, robusto en nombres de shapefiles)
suppressWarnings({
...
ruta_pob     <- file.path(data_dir, "051_DANE_Proyecciones_P_total.rds")  # opcional si luego quieres ponderar
...
ui <- fluidPage(
  ...
  div(class="wrap",
      ...
      div(class="data-note","Exploración del IDM a nivel municipal / departamental."),
      tabsetPanel(
        tabPanel("Exploración IDM", br(),
                 ...
                 fluidRow(
                   column(6, div(class="card",
                                 div(class="card-title","Mapa — IDM"),
                                 leafletOutput("map_idm", height=700))),
                   column(6,
                          div(class="card", div(class="card-title","Top-10 municipios por IDM"),
                              plotlyOutput("bar_top", height=310)),
                          div(class="card", div(class="card-title","Promedio por departamento"),
...
server <- function(input, output, session){
    ...
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      shp <- dep_sf %>% dplyr::left_join(dd, by="COD_DPTO2") %>%
        dplyr::mutate(
          ...
          etq = paste0("<b>", nombre, "</b><br>IDM: ", round(valor,2))
        )
      pal <- make_pal_bin(shp$valor, "YlGnBu")
      
      leafletProxy("map_idm", data = shp) %>% clearShapes() %>% clearControls() %>%
      ...
      shp <- mun_sf %>% dplyr::filter(COD_DPTO2 == sel_cod) %>%
        ...
        dplyr::mutate(
          ...
          etq = paste0("<b>", MUNICIPIO, "</b><br>IDM: ", round(valor,2))
        )
  ...
  observeEvent(input$map_idm_shape_click, {
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      # nombre como viene del shapefile
      nom_shp <- dep_sf$DEPARTAMENTO_N[match(cod, dep_sf$COD_DPTO2)]
      req(!is.na(nom_shp), nzchar(nom_shp))
      
      # normalizamos y buscamos el nombre EXACTO que está en el combo del año elegido
      deps_year <- idm %>%
  ...
  output$bar_top <- renderPlotly({
    ...
    plot_ly(d1 %>% dplyr::arrange(val),
      ...
      layout(xaxis = list(title = "IDM"), yaxis = list(title = "", automargin = TRUE),
  ...
  output$bar_depto <- renderPlotly({
    ...
    plot_ly(d2 %>% dplyr::arrange(val),
      ...
      layout(xaxis=list(title="Promedio IDM"), yaxis=list(title=""), margin=list(l=10,r=10,b=10,t=10))
  })
}
